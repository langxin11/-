...
当预测时域较长时，求解 MPC 问题的计算开销可能很高，使其在许多实时控制问题中不切实际。在这种情况下，可以使用深度 RL 来压缩 MPC 策略。文献~\cite{zhang2016learning} 利用专家 MPC（expert MPC）结合引导策略搜索（guided policy search）控制 MAV，不仅相比专家 MPC 显著减少了计算时间，还无需显式状态估计。文献~\cite{chen2018approximating} 提出一种受约束的神经网络结构来模拟显式 MPC 控制律，并进一步开发了策略梯度方法（policy gradient），其优势函数利用是通过利用模型预测控制问题中的终端成本函数计算得出的。需要指出的是，在关键的约束优化问题中使用神经网络表示最优策略，必然需要验证方法来评估闭环系统的性能。文献~\cite{hu2020reach} 的工作展示了使用半定规划计算一个由神经网络控制的6自由度四旋翼模型的10步前向可达集。
